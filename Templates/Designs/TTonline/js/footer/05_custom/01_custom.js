...
$(document).ready( function(){
  if(navigator.userAgent.includes("Firefox"))
  {
    ...
    // possible fix for mobile menu on firefox
    if($(window).width() < 999)
    {
    	$("a#nav-button").on("click" , function(event){
        	event.preventDefault();
          	$("#mobile-nav").toggleClass("mm-current mm-opened");
          	$("body").toggleClass("mm-opened mm-background mm-theme-dark mm-effect-slide mm-border-full mm-opening");
        });
    } else { $("#mobile-nav").addClass("hidden"); }
  }
